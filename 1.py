...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, input_Q, input_K, input_V, attn_mask):
        ...
        V = self.w_q(input_V).view(batch_size, -1, n_heads, d_v).transpose(1,2)
        # attn_mask : [batch_size, n_heads, seq_len, seq_len]
        attn_mask = attn_mask.unsqueeze(1).repeat(1,n_heads, 1, 1)
        context, attn = ScaledDotProductAttention()(Q, K, V, attn_mask)
        context = context.transpose(1,2).reshape(batch_size, -1, n_heads*d_v)
        out = self.fc(context)
        return nn.LayerNorm(d_model)(out+res), attn
...
class PoswiseFeedForwardNet(nn.Module):
    ...
    def forward(self, inputs):
        ...
        '''
        res = inputs
        return nn.LayerNorm(d_model)(res + self.fc(inputs))

class EncoderLayer(nn.Module):
    def __init__(self):
        super(EncoderLayer, self).__init__()
        self.multi_headattn = MultiHeadAttention()
    ...
    def forward(self, enc_inputs, enc_self_attn_mask):
        ...
        '''
        enc_output, attn = self.multi_headattn(enc_inputs, enc_inputs, enc_inputs, enc_self_attn_mask)
        out = self.ffn(enc_output)
        return out, attn
...
class Encoder(nn.Module):
    def __init__(self):
        ...
        self.pos = nn.Embedding.from_pretrained(get_sinusoid_encoding_table(src_vocab_size, d_model), freeze=True)
    ...
    def forward(self, enc_inputs):
        ...
        '''
        emb_out = self.emb(enc_inputs)
        emb_pos = self.pos(enc_inputs)
        enc_self_attn_mask = get_attn_pad_mask(enc_inputs, enc_inputs) # [batch_size, src_len, src_len]

        enc_out = emb_out + emb_pos
        attens = []
        for layer in self.layers:
            enc_out, attn = layer(enc_out, enc_self_attn_mask)
            attens.append(attn)
        return enc_out, attens
